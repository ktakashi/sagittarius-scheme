...
(define (header-c++ out)
  ...
  (format out "#define INSN_VALUE_MASK 0xFFF~%")
...
(define (footer-c++ out)
  ...
  (format out "  do {				\\~%")
  (format out "    (v1) = ((((int)(insn)) >> INSN_VALUE1_SHIFT) & INSN_VALUE_MASK);	\\~%")
  (format out "    (v2) = ((((int)(insn)) >> INSN_VALUE2_SHIFT) & INSN_VALUE_MASK);	\\~%")
  (format out "  } while (0)~%")
...
(define (body-c++ in out)
  (let ((insts '()))
    ...
    (let loop ((s (read in))
	  ...
	  (begin
	    (match s
	      ((define-inst name inst-values param-count ':label src? . value)
	       (if (not (null? value))
		   (set! count (car value)))
	       (set! insts (cons (list name count param-count src? #t) insts))
	       (format out "  ~s = 0x~2'0x,~%" name count))
	      ((define-inst name inst-values param-count src? . value)
	       (if (not (null? value))
		   (set! count (car value)))
	       (set! insts (cons (list name count param-count src? #f) insts))
	       (format out "  ~s = 0x~2'0x,~%" name count))
	...
	(begin
	  (match s
	    ...
	    ((define-inst name inst-values param-count src? . value)
	     ...
	     (set! *insns* (acons name count *insns*))
	     (format out "  static InsnInfo ~s_INSN = {~s, ~a, ~a, ~a, ~s, ~s};~%"
		     name (symbol->string name) count inst-values param-count 'FALSE (if src? 'TRUE 'FALSE)))
	    ((define-comment comment)
	     (set! count (- count 1))))
	  (loop (read in) (+ count 1)))))
	  ...
	  (begin
	    (match s
	      ((define-inst name inst-values param-count ':label src? . value)
	       (if (not (null? value))
		   (set! count (car value)))
	       (set! insts (cons (list name count inst-values param-count src? #t) insts))
	       (format out "(define-constant ~s ~a)~%" name count))
	      ((define-inst name inst-values param-count src? . value)
	       (if (not (null? value))
		   (set! count (car value)))
	       (set! insts (cons (list name count inst-values param-count src? #f) insts))
	       (format out "(define-constant ~s ~a)~%" name count))
	      ((define-comment comment)
	       (set! count (- count 1)))
	      )
...
(define (main args)
  (if (< (length args) 3)
      (begin
	(print "usage: gen-instruction.scm {scheme|c++} definition-file &output-file &c++-body-file")
	-1)
      (let ((has-out? (>= (length args) 4)))
	(let ((out (if has-out?
		       (open-output-file (cadddr args))
		       (current-output-port))))
	  (gen-file (string->symbol (cadr args)) (caddr args) out (cddddr args))))))
;;;; end of file
;; Local Variables:
;; coding: utf-8-unix
;; End
