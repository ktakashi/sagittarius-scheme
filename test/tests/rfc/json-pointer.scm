...
{\
  ...
  (let ()
    (define (test-pointer-default p j default)
      (test-equal default (p j default))
      (test-equal "mutable" default (p (json->mutable-json j) default)))
    (test-pointer-default (json-pointer "/foo/-") json #f)
    (test-pointer-default (json-pointer "/foo/2") json 'value)))
