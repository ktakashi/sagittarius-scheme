...
(define input-file 
  (build-path 
   (current-directory)
   "test/data/testvectors/json_web_encryption_test.json"))
...
(define (run-tests group)
  (define ((run-test jwk) test)
    (define id (tcid-pointer test))
    (define comment (comment-pointer test))
    (define pt (pt-pointer test))
    (define enc (enc-pointer test))
    (define jwe (jwe-pointer test))
    (define result (result-pointer test))
    (define flags (flags-pointer test))

    (define (encryptor&decryptor jwk)
      (case (jwk-alg jwk)
	((A128KW A192KW A256KW A128GCMKW A192GCMKW A256GCMKW)
	 (values (make-aeskw-jwe-encryptor jwk)
		 (make-aeskw-jwe-decryptor jwk)))
	((ECDH-ES+A128KW ECDH-ES+A192KW ECDH-ES+A256KW ECDH-ES)
	 (values (make-ecdh-es-jwe-encryptor jwk)
		 (make-ecdh-es-jwe-decryptor jwk)))
	((RSA-OAEP RSA-OAEP-256 RSA1_5)
	 (values (make-rsa-jwe-encryptor jwk)
		 (make-rsa-jwe-decryptor jwk)))
	;; dir
	(else
	 (values (make-direct-jwe-encryptor jwk)
		 (make-direct-jwe-decryptor jwk))
	 )))
    (define (parse-jwe jwe)
      (and (not (member "JsonSerialization" flags))
	   (not (member "WrongCipher" flags))
	   (not (member "Pkcs15WithOaepKey" flags))
	   (jwe:parse jwe)))
    (let-values (((encryptor decryptor) (encryptor&decryptor jwk)))
      (if (string=? result "valid")
	  (let ()
	    (define jwe-object (parse-jwe jwe))
	    (when jwe-object
	      (test-equal (list id comment) (hex-string->bytevector pt)
			  (jwe:decrypt decryptor jwe-object))
	      (test-equal (list id comment) (hex-string->bytevector pt)
			  (jwe:decrypt decryptor
			    (jwe:encrypt encryptor (jwe-object-header jwe-object)
					 (hex-string->bytevector pt))))))
	  (test-error (list id comment)
		      condition?
		      (let ()
			(define jwe-object (parse-jwe jwe))
			(jwe:decrypt decryptor jwe-object))))))
  ...
  (test-group (list type comment)
   (for-each (run-test (json->jwk (private-pointer group)))
	     (tests-pointer group))))
