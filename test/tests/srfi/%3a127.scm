(import (rnrs)
	(srfi :127)
	(rename (srfi :121) (generator make-generator))
	(prefix (srfi :64) srfi64:))
...
(define-syntax test
  (syntax-rules ()
    ...
    ((_ name expect expr)
     (srfi64:test-equal name expect expr))))

(define-syntax test-assert
  (syntax-rules ()
    ((_ expr)
     (srfi64:test-assert 'expr expr))))

(define-syntax test-error
  (syntax-rules ()
    ((_ expr)
     (srfi64:test-error 'expr condition? expr))))


(srfi64:test-begin "SRFI-127 Lazy sequence")
...
(test-group "lseqs"
  ...
  (test-group "lseqs/searching"
    ...
    ;; extra
    (test #t (lseq-every < '(1 1 1 1 1) '(2 7 3 8 2)))
