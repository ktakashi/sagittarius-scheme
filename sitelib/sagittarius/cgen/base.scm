...
#!compatible
  ...
  ;; (ash 1 1) -> 1 << 1
  ;; (ash 1 -1) -> 1 << -1
  (define (ash body dispatch k)
    (let ((args (cdr body))
	  (save (renderer-no-indent)))
      (unless (= (length args) 2)
	(error 'ash (format "2 arguments required but got ~a" (length args)) body))
      (renderer-no-indent #t)
      ((renderer) "(")
      (renderer-no-indent #t)
      (dispatch (car args) dispatch k)
  ...
  (define (negate-proc body dispatch k)
	...
	(error 'ash (format "1 arguments required but got ~a" (length args)) body))
      (renderer-no-indent #t)
      ((renderer) "-")
      (renderer-no-indent #t)
      (dispatch (car args) dispatch k)
  ...
  (define (init)
    ...
    (hashtable-set! *dispatch-table* 'ash ash)
    (hashtable-set! *dispatch-table* 'logand logand)
