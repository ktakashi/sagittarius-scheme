...
(define (read-symbol in ls)
  (do ((ls ls (cons c ls)) (c (peek-char in) (peek-char in)))
    ...
    (read-char in)))
;; Sagittarius 
(define (read-keyword in ls)
  (make-keyword (read-symbol in ls)))
   ...
   (else
    (case ch
      ((#\( #\[ #\{)
       (let lp ((res '()))
         (let ((x (scrib-read in)))
           (cond ((eof-object? x) (error 'scrib-read "unterminated list" x))
	 ...
	 ((#\v)
	  (read-char in)
	  (unless (and (eqv? #\u (read-char in))
		       (eqv? #\8 (read-char in))
		       (eqv? #\( (peek-char in)))
	    (error 'scrib-read "unknown # syntax"))
	  (u8-list->bytevector (scrib-read in)))
         (else (error 'scrib-read "unknown # syntax"))))
      ;; added
      ((#\:)
       (read-keyword in '()))
      (else
      ...
      (cond
        ((eof-object? c)
         (if (zero? depth)
             ...
             (error 'scribble-parse "unterminated expression" punc)))
