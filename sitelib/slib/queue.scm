;; -*- scheme -*-
;; queue.scm - queue implementation from slib ported by Takashi Kato
(library (slib queue)
    (export make-queue
	    queue?
	    queue-empty?
...
(define-record-type queue
  (fields (mutable first-pair queue:first-pair queue:set-first-pair!)
	  (mutable last-pair queue:last-pair  queue:set-last-pair!))
  (protocol
   (lambda (p)
     (lambda ()
       (p '() '())))))
...
(define (queue-rear q)
  (let ((last-pair (queue:last-pair q)))
    (if (null? last-pair)
	(error 'queue-rear "queue is empty" q))
    (car last-pair)))
)