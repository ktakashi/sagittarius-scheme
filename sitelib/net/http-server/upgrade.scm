...
(library (net http-server upgrade)
    (export http-server:connection?
            ...
            http-server:make-server-http-connection
    ...
    (import (rnrs)
            ...
            (only (net http-server http2) *http-server:http2-driver*)
            (only (net http-server protocol) http-server:select-protocol-driver)
...
(define (http-server:make-server-http-connection server socket)
  (let* ((protocol-registry (slot-ref server 'registry))
    ...
    (if (eq? driver *http-server:http2-driver*)
        (make-http-server:http2-upgrade-connection
         server socket #vu8() 0 #f driver #f protocol-registry upgrade-registry)
        (make-http-server:http1-connection
         server socket #vu8() 0 #f driver #f protocol-registry upgrade-registry))))
