;;; SRFI-14 ported for Sagittarius Scheme by Takashi Kato
#!compatible
...
(define (%default-base maybe-base proc)
  (if (pair? maybe-base)
	...
	(if (null? tail)
		...
		(error '%default-base "BASE-CS parameter not a char-set" proc bcs))
	    (error '%default-base "Expected final base char set -- too many parameters"
		   proc maybe-base)))
...
(define (%char-set:s/check cs proc)
  (let lp ((cs cs))
	...
	(error '%char-set:s/check "Not a char-set" proc cs))))
...
(define (char-set-hash cs . maybe-bound)
  (let* ((bound (get-optional maybe-bound 4194304 (lambda (n) (and (integer? n)
								(exact? n)
								(<= 0 n)))))
...
(define (%ucs-range->char-set! lower upper error? bs proc)
  ...
  (if (and (< lower upper) (< 256 upper) error?)
      (error 'ucs-range->char-set! "Requested UCS range contains unavailable characters -- this implementation only supports Latin-1"
	     lower upper))
