...
(library (text sxml ssax)
    ...
    (import (except (rnrs) fold-right) ;; because of defferent behaviour
	    (sagittarius io)
	    (text parse)
	    (text sxml helper)
	    (only (srfi :1 lists) fold-right fold)
	    (srfi :8 receive)
	    (srfi :13 strings))
...
(define ssax:complete-start-tag
 ...
 (let ((xmlns (string->symbol "xmlns"))
  ...
  (define (validate-attrs port attlist decl-attrs)
    ...
    (define (add-default-decl decl-attr result)
      (receive (attr-name content-type use-type default-value)
    ...
    (let loop ((attlist attlist) (decl-attrs decl-attrs) (result '()))
	  ...
	  (receive (attr attr-others) (attlist-remove-top attlist)
	    (receive (decl-attr other-decls)
 ...
 (lambda (tag-head port elems entities namespaces)
   (receive (attlist) (ssax:read-attributes port entities)
     (receive (empty-el-tag?)
	 (begin
	   (ssax:skip-S port)
	   (and
	    (eqv? #\/ 
       ...
       (receive (elem-content decl-attrs)	; see xml-decl for their type
	 ...
	 (receive (merged-attrs) (if decl-attrs (validate-attrs port attlist decl-attrs)
				     (attlist->alist attlist))
	   (receive (proper-attrs namespaces)
  ...
  (syntax-rules ()
   ...
   (lambda (start-tag-head port elems entities namespaces
     ...
     (let handle-start-tag ((start-tag-head start-tag-head)
       ...
       (receive (elem-gi attributes namespaces expected-content)
