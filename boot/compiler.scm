  ...
  (smatch form
    ((- ((name trans-spec) ___) body ___)
     (let* ((ids (collect-lexical-id name p1env))
	    ...
	    (unrenamed-spec (unrename-expression trans-spec ids))
	    (newenv (p1env-extend p1env
				  (%map-cons unrenamed-ids trans-spec) LEXICAL))
	    (trans (map (lambda (n x)
			  (pass1/eval-macro-rhs
			   'letrec-syntax
			   (variable-name n)
    ...
    (let loop ((vars vars)
      ...
      (cond ((pair? vars)
	     (loop (cdr vars)
		   (loop (car vars) r)))
	    ((and (identifier? vars)
		  ;; we must not collect pvar.
		  ;; if p1env-pvar-lookup returned not number,
		  ;; it's not pvar.
	 ...
	 ((:rest)
	  ...
	  (receive (rs xs) (collect-args xs '())
	    (smatch rs
	      ...
	      (_ 
	       (syntax-error
		":rest keyword in the extended lambda form must be followed by exactly one argument" kargs)))))
	 ((:allow-other-keys)
	  ...
	  (receive (a xs) (collect-args xs '())
	    (smatch a
	      ...
	      (_ (syntax-error
		  ":allow-other-keys keyword in extended lambda form can be followed by zero or one argument" kargs)))))
...
(define-pass1-syntax (lambda form p1env) :null
  (smatch form
    ((- formals . body) (pass1/lambda form formals body p1env #t))
  ...
  (smatch form
    ((- ((vars expr) ___) body ___)
     ...
     (let loop ((vars vars)
		...
		(ids '()))
       (if (null? vars)
	   (pass1/body (unrename-expression body ids)
...
(define-pass1-syntax (and-let* form p1env) :sagittarius
  ...
  (smatch form
    ((_ binds . body) (process-binds binds body p1env))
    (_ (syntax-error "malformed and-let*" form)))
  )
    ...
    (smatch form
	 ...
	 ($let form 'rec lvars
	       (map (lambda (lv init)
		    ...
		    ;; its bounded id  inside.
		    lvars (unrename-expression expr ids))
	       (pass1/body (unrename-expression body ids) newenv))))
  ...
  (smatch form
    ((- ((var init . update) ___) (test expr ___) body ___)
     (let* ((tmp (make-lvar 'do-proc))
	    ...
	    (unrenamed-ids (unrename-expression var ids))
	    (args (imap make-lvar+ unrenamed-ids))
	    (newenv (p1env-extend/proc p1env (%map-cons unrenamed-ids args)
				       LEXICAL 'do-proc))
	    (clo ($lambda 
		  form
