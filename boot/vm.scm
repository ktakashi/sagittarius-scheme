...
(define *stack* (make-parameter (make-vector 1000)))
  ...
  (let-syntax ((next (syntax-rules ()
    ...
    (case/unquote (get-insn insn)
	 ...
	 (VM x (skip) n c f s)))
      ...
      ((CONST_PUSH)
       (let ((value (fetch)))
	 (VM x (skip) a c f (push value s))))
      ((CONSTI_PUSH)
       (let ((value (insn-value1 insn)))
	 ...
	 (let ((a (refer-local n)))
      ...
      ((LREF_CAR_PUSH)
       (let ((n (insn-value1 insn)))
	 (let ((a (refer-local n)))
	   (VM x (skip) (car a) c f (push (car a) s)))))
      ((LREF_CDR_PUSH)
       (let ((n (insn-value1 insn)))
	 (let ((a (refer-local n)))
	 ...
	 (VM x (skip) (/ n a) c f s)))
      ...
      ((PAIRP)
       (VM x (skip) (pair? a) c f s))
      ((SYMBOLP)
       (VM x (skip) (symbol? a) c f s))
      ((CAR)
       (VM x (skip) (car a) c f s))
      ((CDR)
       (VM x (skip) (cdr a) c f s))
      ...
      ((CDR_PUSH)
       (VM x (skip) (cdr a) c f (push (cdr a) s)))
      ((CAAR)
       (VM x (skip) (caar a) c f s))
      ((CADR)
       (VM x (skip) (cadr a) c f s))
      ((CDAR)
       (VM x (skip) (cdar a) c f s))
      ((CDDR)
       (VM x (skip) (cddr a) c f s))
      ((CONS)
       (VM x (skip) (cons (index s 0) a) c f (- s 1)))
      ((CONS_PUSH)
       (VM x (skip) (cons (index s 0) a) c f (push (cons (index s 0) a) (- s 1))))
      ;; vector
      ((VECTOR)
	 ...
	 (VM x (skip) v c f (- s (if (> val1 0) n 0)))))
      ...
      ((SET_CAR)
       (VM x (skip) (set-car! (index s 0) a) c f (- s 1)))
      ((SET_CDR)
       (VM x (skip) (set-cdr! (index s 0) a) c f (- s 1)))
      ((LIST)
...
(define (shorten-object o)
  (cond ((closure? o)
	 ...
	 (if (circular-list? o)
		 ...
		 (let ((s (x->string o)))
		   (if (> (string-length s) 30)
		       (substring s 0 30)
		       s)))
	     (cons (shorten-object (car o))
		   (shorten-object (cdr o)))))
	((box? o)
	 (format "#<box ~a>"
	...
	((vector? o)
	 (format/ss "#<code? ~a>" (vector-ref o 0)))
	(else
	 o)))
	...
	(case o
	  ...
	  ((c)
	   ...
	   (let* ((lib (cadr opt))
		  ...
		  (c (compile s '())))
	     (print (format "generating compiled library ~s" lib))
	     #;(print (map (lambda (imported)
			   (shorten-object (car imported)))
			 (library-imported (find-library (read (open-input-string lib)) #f))))
...
(define (apply-proc proc . args)
  (parameterize ((*stack* (make-vector 1000)))
    (VM `#(,FRAME 6
	   ,CONST_PUSH ,proc
	   ,CONST ,@args
	   ,(merge-insn1 APPLY 2)
...
(add-namespace! bitwise-arithmetic-shift (a b))
(add-namespace! bitwise-arithmetic-shift-left (a b))
(add-namespace! bitwise-arithmetic-shift-right (a b)
		(lambda (a b)
		  (ash a (- b))))
(add-namespace! ash (a b))
...
(add-namespace! caaar  (x))
(add-namespace! caadr  (x))
(add-namespace! cadar  (x))
(add-namespace! caddr  (x))
(add-namespace! cdaar  (x))
(add-namespace! cdadr  (x))
(add-namespace! cddar  (x))
(add-namespace! cdddr  (x))
(add-namespace! caaaar (x))
(add-namespace! caaadr (x))
(add-namespace! caadar (x))
(add-namespace! caaddr (x))
(add-namespace! cadaar (x))
(add-namespace! cadadr (x))
(add-namespace! caddar (x))
(add-namespace! cadddr (x))
(add-namespace! cdaaar (x))
(add-namespace! cdaadr (x))
(add-namespace! cdadar (x))
(add-namespace! cdaddr (x))
(add-namespace! cddaar (x))
(add-namespace! cddadr (x))
(add-namespace! cdddar (x))
(add-namespace! cddddr (x))
...
(cond-expand
 ...
 (sagittarius
  (add-namespace! make-eq-hashtable () make-eq-hashtable)
  ;; only used for *libraries*
  (add-namespace! make-hashtable (a b) 
		  (lambda (h e) (make-equal-hashtable)))
  (add-namespace! hashtable-ref (h k) hashtable-ref)
  (add-namespace! hashtable-set! (h k v) hashtable-set!)
  (add-namespace! hashtable->alist (h) hashtable->alist)
  (add-namespace! hashtable-keys-list (h) hashtable-keys-list)
  (add-namespace! hashtable-values (h) hashtable-values-list)
  (add-namespace! hashtable-values-list (h) hashtable-values-list)
  (add-namespace! x->string (o) (lambda (o)
				  (format "~a" o))) ;; for vm-dump-code
  ))
...
(define (notify-cond-expand form . marks)
  (let loop ((form form)
	     (r '()))
    (if (null? form)
	(reverse r)
...
(define (construct-library file lib imports oexports)
  (let* ((sexp (file->sexp-list file))
	 (program (notify-cond-expand sexp 'sagittarius 'sagittarius.scheme.vm))
	 (exports '()))
