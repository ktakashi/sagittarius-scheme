	       ...
	       (cond ((closure-option? cl)
			...
			(cond ((zero? extra-argc)
				 ...
				 (VM (closure-body-code cl)
				     ...
				     cl
				     cl
				     (- sp (+ required-length 1))
				     sp)))
			      ((> extra-argc 0)
			       (index-set! s (- extra-argc 1)
					   (stack->pair-args s extra-argc))
			       (let ((sp (- s (- extra-argc 1))))
				 (VM (closure-body-code cl)
				     (closure-body-pc cl)
				     cl
				     cl
...
(define (vm-dump-code cb . inner)
  ...
  (let ((data (array-data (code-builder-code cb)))
    ...
    (format #t "~asize: ~a, ~a, ~a~%" (if (null? inner) "" (car inner)) length
	    (code-builder-argc cb) (code-builder-optional? cb))
