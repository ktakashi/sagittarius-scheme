	    ...
	    (for-each (lambda (pos)
			...
			(cond ((identifier? object)
				 ...
				 (set! lib-info (cons (id-library object) lib-info)))
				   ...
				   (let ((s (memq (id-name object) uninterned)))
				     (if s
					 (format out "WORD(SG_CDR(Sg_Assq(STRING(\"~a\"), uninterned)));~%" (id-name object))
					 (begin
					   (set! uninterned (cons (id-name object) uninterned))
					   (format out "tmp = UNSIDENT(~s, lib);~%" (symbol->string (id-name object)))
					   (format out "  uninterned = Sg_Acons(STRING(\"~a\"), SG_OBJ(tmp), uninterned);~%"
						   (symbol->string (id-name object))))))))
			      ((symbol? object)
