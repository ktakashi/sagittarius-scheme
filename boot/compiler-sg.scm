...
(define-pass1-syntax
 ...
 (smatch
  ...
  ((- ((name trans-spec) ___) body ___)
   (let*
    ((ids (collect-lexical-id name p1env))
     (unrenamed-ids (unrename-expression name ids))
     (unrenamed-spec (unrename-expression trans-spec ids))
     (trans
      (map
       (lambda
        (n x)
  ...
  ((- ((name trans-spec) ___) body ___)
   (let*
    ((ids (collect-lexical-id name p1env))
     ...
     (unrenamed-spec (unrename-expression trans-spec ids))
     (newenv
      (p1env-extend p1env (%map-cons unrenamed-ids trans-spec) LEXICAL))
     (trans
      (map
       (lambda
        (n x)
...
(define
 ...
 (lambda
  ...
  (let
   ...
   ((vars vars) (r '()))
   (cond
    ((pair? vars) (loop (cdr vars) (loop (car vars) r)))
    ((and (identifier? vars) (variable? (p1env-pvar-lookup p1env vars)))
     (cons vars r))
    (else r)))))
...
(define
 ...
 (lambda
  ...
  (receive
   ...
   (let*
    ((ids (collect-lexical-id vars p1env))
 ...
 (smatch
  ...
  ((- formals expr body ___)
   (receive
    ...
    (let*
     ((ids (collect-lexical-id args p1env))
   ...
   (let*
    ...
    (let*
     ((env1 (p1env-extend p1env `((,name unquote lvar)) LEXICAL))
      (env2
       (p1env-extend/name env1 (%map-cons unrenamed-ids args) LEXICAL name))
      (lmda
       ($lambda
        form
        name
...
(define-pass1-syntax
 ...
 (smatch
  ...
  ((- ((var expr) ___) body ___)
   (let
    loop
    ((vars var) (inits expr) (p1env p1env) (src form) (ids '()))
    (if
     (null? vars)
     ...
     (let*
      ((id (collect-lexical-id (car vars) p1env))
       ...
       (iexpr
        (pass1
         (unrename-expression (car inits) ids)
        ...
        (case
         ...
         ((include include-ci)
          (let
           ...
           ($seq-body-set!
            seq
            (append ($seq-body seq) (list (pass1 (car expr) p1env))))
           (loop (cdr clauses))))
         ((begin)
          ($seq-body-set!
           seq
...
(define (pass1/open-include-file path includer-path)
  (define (check path)
    ...
    (syntax-error
     ...
     path))(cond
   ((absolute-path? path) (or (check path) (bad)))
   ((and includer-path (check (build-path includer-path path))))
   ((check path))
   (else (bad))))
...
(define-pass1-syntax
 ...
 (smatch
  form
  ((- files ___)
   (let ((form (pass1/include files p1env #f))) (pass1 (car form) p1env)))
...
(define-pass1-syntax
 ...
 (smatch
  form
  ((- files ___)
   (let ((form (pass1/include files p1env #t))) (pass1 (car form) p1env)))
  (- (syntax-error "malformed include" form))))
          ...
          ((identifier? head)
           (let
            ...
            (if
             ...
             (pass1/body-finish intdefs intmacros exprs p1env))))
          (else (scheme-error 'pass1/body "[internal] pass1/body" op head))))
        (else #f))))
     (pass1/body-finish intdefs intmacros exprs p1env)))
   (- (pass1/body-finish intdefs intmacros exprs p1env)))))
   ...
   (cond
    ...
    (else
     (receive
      ...
      (let
       loop
       ((exprs intmacros) (env newenv) (ids '()))
       (if
        (null? exprs)
        (values env ids)
        (case
...
(define (pass2/lambda-lifting iform library)
  (if
   ...
   (let
    ...
    (let
     ((lambda-nodes (label-dic-info dic)))
     (if
      (or
       (null? lambda-nodes)
       (and
        (null? (cdr lambda-nodes))
        ($lambda-lifted-var (car lambda-nodes))))
      iform
      (let
       ((lifted (pass2/lift lambda-nodes library)))
       (if
        (null? lifted)
        iform
        (let
         ((iform. (pass2/subst iform (make-label-dic '()))))
     ...
     (smatch
      ...
      ((accessor expr)
       (let
        ...
        `(let*
         ((,org (,accessor ,expr)) (,result (pass2/subst ,org ,labels)))
         (unless (eq? ,org ,result) (,setter ,expr ,result))
         ,expr)))))))))
(define-syntax
 pass2/subst*!
 (er-macro-transformer
...
(define (pass2-scan/$DEFINE iform bs fs t? labels)
  (unless
   ...
   (error
    'pass2/lambda-lifting
    "[internal] $DEFINE in non-toplevel"))(pass2/scan
   ($define-expr iform)
   bs
   fs
   #t
...
(define (pass2-scan/$LSET iform bs fs t? labels)
  (let
   ((fs (pass2/scan ($lset-expr iform) bs fs t? labels)))
   (pass2/add-lvar ($lref-lvar iform) bs fs)))
...
(define (pass2-scan/$IF iform bs fs t? labels)
  (let*
   ((fs (pass2/scan ($if-test iform) bs fs t? labels))
...
(define (pass2-scan/$LET iform bs fs t? labels)
  (let*
   ((new-bs (append ($let-lvars iform) bs))
...
(define (pass2-scan/$RECEIVE iform bs fs t? labels)
  (let
   ((fs (pass2/scan ($receive-expr iform) bs fs t? labels))
...
(define (pass2-scan/$LAMBDA iform bs fs t? labels)
  (let
   ((inner-fs
     (pass2/scan ($lambda-body iform) ($lambda-lvars iform) '() #f labels)))
   (unless
   ...
   (cond
    ...
    (else
     ...
     (let
      loop
      ((inner-fs inner-fs) (fs fs))
      (if
       (null? inner-fs)
       fs
       (loop (cdr inner-fs) (pass2/add-lvar (car inner-fs) bs fs))))))))

(define (pass2-scan/$LABEL iform bs fs t? labels)
  (cond
   ((label-seen? labels iform) fs)
   (else
     ...
     (or
      (and
       ...
       (let
        ...
        (and n (set! top-name (if (identifier? n) (id-name n) n)))))
      (loop (cdr lms)))))
   (let
    ((results '()))
    (let
     ...
     (cond
      ...
      (else
       (let*
        ((lm (car lms)) (fvs ($lambda-free-lvars lm)))
        (when
         (or
          (null? fvs)
          (and
         ...
         (let
          ((gvar (make-identifier (gensym) '() library)))
          ($lambda-name-set!
           lm
           (list top-name (or ($lambda-name lm) (id-name gvar))))
...
(define (pass2-subst/$LREF iform labels)
  (or
   (and
    (= (lvar-set-count ($lref-lvar iform)) 0)
    (let
...
(define (pass2-subst/$IF iform labels)
  (pass2/subst!
   ...
   labels)(pass2/subst!
   ($if-then iform)
   labels)(pass2/subst! ($if-else iform) labels))

(define (pass2-subst/$LET iform labels)
  (pass2/subst*!
   ($let-inits iform)
   labels)(pass2/subst! ($let-body iform) labels))

(define (pass2-subst/$RECEIVE iform labels)
  (pass2/subst!
   ($receive-expr iform)
   labels)(pass2/subst! ($receive-body iform) labels))

(define (pass2-subst/$LAMBDA iform labels)
  (pass2/subst!
   ($lambda-body iform)
   labels)(or
   (let ((id ($lambda-lifted-var iform))) (and id ($gref id)))
   iform))
...
(define (pass2-subst/$LABEL iform labels)
  (unless
   ...
   (label-push! labels iform)
   (pass2/subst! ($label-body iform) labels))iform)

(define (pass2-subst/$SEQ iform labels)
  (pass2/subst*! ($seq-body iform) labels)iform)

(define (pass2-subst/$CALL iform labels)
  (pass2/subst*!
   ($call-args iform)
   labels)(pass2/subst! ($call-proc iform) labels))

(define (pass2-subst/$ASM iform labels)
  (pass2/subst*! ($asm-args iform) labels)iform)

(define (pass2-subst/$LIST iform labels)
  (pass2/subst*! ($*-args iform) labels)iform)
...
(define
 ...
 (lambda
  (sets new-sets)
  (if (null? new-sets) sets (lset-union eq? sets new-sets))))
   ...
   (lambda
    ...
    (cond
     ...
     ((has-tag? i $CALL)
      (if
       (and
        (eq? ($call-flag i) 'embed)
        (has-tag? ($lambda-body ($call-proc i)) $LABEL)
        (assq
         ($label-label ($lambda-body ($call-proc i)))
...
(define
 ...
 (lambda
  ...
  (when (memq ($lref-lvar iform) (renv-sets renv)) (cb-emit0! cb UNBOX))
  0))
   ...
   (let*
    ((new-renv
     ...
     (assign-size
      (let
       loop
       ((args args) (vars vars) (size 0) (index (length (renv-locals renv))))
       (cond
        ((null? args) size)
        (else
         (let
...
(define
 ...
 (lambda
  ...
  (let
   ...
   (let*
    ((renv (if tail? renv (renv-add-frame-dummy renv)))
 ...
 (lambda
  ...
  (let*
   ((proc ($call-proc iform))
    ...
    (sets (pass3/find-sets body vars))
    (frlen (length free)))
   ($call-renv-set! iform (length (renv-locals renv)))
   (let
    ((args-size
      (pass3/compile-args
       args
    ...
    ((APPLY)
     (if
      ...
      (let
       ...
       (let*
        ((renv (renv-add-frame-dummy renv))
  ...
  (case
   ...
   ((2)
    (let
     ...
     (cb-emit0! cb PUSH)
     (let
      ((d1 (pass3/rec (cadr args) cb (renv-add-dummy renv) 'normal/top)))
      (pass3/emit-asm! cb insn info)
      (max d0 (+ d1 1)))))
   (else
    (let
     loop
     ((args args) (depth 0) (count 0) (renv renv))
     (cond
      ((null? (cdr args))
       (let
        ((d (pass3/rec (car args) cb renv 'normal/top)))
    ...
    ((- info code param arg0 arg1)
     (let
      ...
      `(let
       ...
       (cb-emit0! cb PUSH)
       (let
        ((,d1 (pass3/rec ,arg1 cb (renv-add-dummy renv) 'normal/top)))
...
(define
 ...
 (lambda
  ...
  (if
   ...
   (let
    loop
    ((as args) (depth 0) (count 0) (renv renv))
    (cond
     ((null? (cdr as))
      (let
 ...
 (lambda
  ...
  (if
   ...
   (let
    ...
    (let
     loop
     ((args (cdr args)) (depth (+ d 1)) (cnt 1) (renv (renv-add-dummy renv)))
     (if
      (null? args)
      depth
      (let
