...
(define variable? (lambda (v) (or (symbol? v) (identifier? v))))
...
(define
 ...
 (lambda
  ...
  (let
   ...
   (or
    mod
    (error 'ensure-library (format "~a: no such library: ~s" name thing))))))
...
(define
 p1env-lookup
 (lambda
  (p1env name lookup-as)
  (let
   ((name-ident? (identifier? name)) (frames (p1env-frames p1env)) (ret #f))
   (let
    loop
    ((fp frames))
    (cond
     ((pair? fp)
      (when
       (and name-ident? (eq? (id-envs name) fp))
       (set! name-ident? #f)
       (set! name (id-name name)))
      (when (> (caar fp) lookup-as) (loop (cdr fp)))
      (let
       loop2
       ((tmp (cdar fp)))
       (if
        ...
        (let
         ((vp (car tmp)))
         (if (eq? name (car vp)) (cdr vp) (loop2 (cdr tmp))))
        (loop (cdr fp)))))
     (else
      (if
       (symbol? name)
...
(define
 ...
 (lambda (x marker) (and (pair? x) (eq? (car x) marker))))
...
(define-pass1-syntax
 ...
 (smatch
  ...
  (else (error 'syntax-error "malformed let-syntax" form))))
...
(define-pass1-syntax
 ...
 (smatch
  ...
  ((- name ((var expr) ___) body ___)
   (unless
    (variable? name)
    (error 'syntax-error "bad name for named let" name))
   (let
    ((lvar (make-lvar name))
...
(define-pass1-syntax
 ...
 (smatch
  form
  ((- name expr)
   (unless (variable? name) (error 'syntax-error "malformed set!" form))
   (let
...
(define
 ...
 (lambda
  ...
  (case
   ...
   ((for) (load-library to from))
   ((:only)
    (case
     (car etc)
     ((:compile) (load-library to from) (library-transient-set! from #t))
     (else (error 'syntax-error "malformed import spec" oform))))
   (else (error 'syntax-error "malformed import spec:" oform)))))
       ...
       (let
        ...
        (cond
         ...
         (else
          (error
           'syntax-error
           "malformed rename clause in export spec"
           oform)))))
      (else
       (error
        'syntax-error
       ...
       (cond
        (head
         ...
         (cond
          ...
          (else (error 'pass1/body "[internal] pass1/body" head))))
        (else #f))))
     (pass1/body-finish intdefs exprs p1env)))
   (- (pass1/body-finish intdefs exprs p1env)))))
...
(define
 ...
 (lambda
  (iform penv tail?)
  (let
   ((if-test ($if-test iform)))
   (cond
    ((has-tag? if-test $CONST)
     (if
      ($const-value if-test)
    ...
    (else
     (let
      ((test-c (pass2/rec ($if-test iform) penv #f))
  ...
  (define
   ...
   (lambda
    ...
    (cond
     ...
     (else (error 'pass3/find-free "unknown iform:" (iform-tag i))))))
  ...
  (define
   ...
   (lambda
    ...
    (cond
     ...
     (else (error 'pass3/find-sets "unknown iform:" (iform-tag i))))))
  (uniq (rec iform '()))))
