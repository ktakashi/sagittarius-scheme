...
(define (pass5/find-free iform locals renv cb)
  ...
  (define (rec i l)
    (cond ((has-tag? i $CONST) '())
	  ((has-tag? i $LET)
	   (append ($append-map1 (lambda (fm) (rec fm l))
				 ($let-inits i))
		   (rec ($let-body i) ($let-lvars i))))
	  ((has-tag? i $RECEIVE)
	   (append (rec ($receive-expr i) l)
		   (rec ($receive-body i) ($receive-lvars i))))
	  ((has-tag? i $SEQ)
	   ($append-map1 (lambda (fm) (rec fm l))
			 ($seq-body i)))
	  ((has-tag? i $LAMBDA)
	   (rec ($lambda-body i) ($lambda-lvars i)))
	  ((has-tag? i $LSET)
	   (let ((lvar ($lset-lvar i)))
	     (if (pass5/exists-in-can-frees? lvar (renv-can-free renv))
		 (cons lvar (rec ($lset-expr i) l))
		 (rec ($lset-expr i) l))))
	  ((has-tag? i $LREF)
	  ...
	  ((has-tag? i $LABEL)
	   (if (memq i labels-seen)
	       '()
	       (begin
		 (set! labels-seen (cons i labels-seen))
		 (rec ($label-body i) l))))
	  ((has-tag? i $IT) '())
	  ((has-tag? i $UNDEF) '())
	  ((has-tag? i $LIST)
	   ($append-map1 (lambda (fm) (rec fm l)) ($*-args i)))
	  (else
...
(define (pass5/find-sets iform lvars)
  ...
  (define (rec i)
    (cond ((has-tag? i $CONST) '())
	  ((has-tag? i $LET)
	   (append ($append-map1 (lambda (init) (rec init))
				 ($let-inits i))
		   (rec ($let-body i))))
	  ((has-tag? i $RECEIVE)
	   (append (rec ($receive-expr i))
		   (rec ($receive-body i))))
	  ((has-tag? i $SEQ)
	   ($append-map1 (lambda (fm) (rec fm)) ($seq-body i)))
	  ((has-tag? i $LAMBDA)
	   (rec ($lambda-body i)))
	  ((has-tag? i $LSET)
	   (let ((lvar ($lset-lvar i)))
	  ...
	  ((has-tag? i $IF)
	   (append (rec ($if-test i))
		   ...
		   (rec ($if-else i))))
	  ((has-tag? i $ASM)
	   ($append-map1 (lambda (arg) (rec arg))
			 ($asm-args i)))
	  ((has-tag? i $DEFINE)
	   (rec ($define-expr i)))
	  ((has-tag? i $CALL)
	   (append ($append-map1 (lambda (arg) (rec arg))
				 ($call-args i))
		   (rec ($call-proc i))))
	  ((has-tag? i $LABEL)
	   (if (memq i labels-seen)
	  ...
	  ((has-tag? i $UNDEF) '())
	  ((has-tag? i $LIST)
	   ($append-map1 (lambda (arg) (rec arg)) ($*-args i)))
	  (else
