...
(define (update-cycle-mark! mark lst seen)
  ...
  (let loop ((lst lst))
    (when (pair? lst)
      (cond ((marked-obj (car lst)) =>
	     (lambda (v) (set-car! lst v) (loop (cdr lst))))
	    ;; if we set cdr, then it's done
	    ((marked-obj (cdr lst)) => (lambda (v) (set-cdr! lst v)))
	    (else (loop (cdr lst)))))))
...
(define (rewrite-form form seen env library
  ...
  (let loop ((expr form) (seen '()))
    (cond ((assq expr seen) => cdr)
	  ((pair? expr)
	   (let* ((mark (make-cyclic-mark expr))
		  (seen (acons expr mark seen)))
	     (let ((a (loop (car expr) seen))
		   (d (loop (cdr expr) seen)))
	       (if (and (eq? a (car expr)) (eq? d (cdr expr)))
		   expr
		   (let ((r (cons a d)))
		     (update-cycle-mark! mark r seen)
		     (syntax-case-source-info r expr))))))
	  ((vector? expr)
	   (let ((len (vector-length expr)))
	     (let lp ((i 0) (vec #f))
	       (if (= i len)
		   (or vec expr)
		   (let ((e (loop (vector-ref expr i) seen)))
...
(define expand-syntax
  (lambda (vars template ranks)
	...
	(let loop ((i 0) (vec #f))
      ...
      (define (update-mark! seen mark lst) (update-cycle-mark! mark lst seen))
	...
	(cond ((assq lst seen) => cdr)
	      ((contain-identifier? lst)
	       (cond ((pair? lst)
			...
			(let ((a (loop (car lst) seen))
			      (d (loop (cdr lst) seen)))
			  (if (and (eq? (car lst) a) (eq? (cdr lst) d))
			      lst
			      (let ((r (cons a d)))
				(update-mark! seen mark r)
