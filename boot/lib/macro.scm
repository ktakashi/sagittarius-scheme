...
(define (rewrite-form form seen env library
  ...
  (define (rec form)
    (let loop ((expr form))
      (cond ((hashtable-ref cycle expr #f))
	    ((pair? expr)
	     (hashtable-set! cycle form expr)
	     (let ((a (loop (car expr)))
		   (d (loop (cdr expr))))
	       (if (and (eq? a (car expr)) (eq? d (cdr expr)))
		   expr
		   (source-info*! (cons a d) (source-info expr)))))
	    ((vector? expr)
	  ...
	  (if (= i len)
		...
		(if (eq? e (vector-ref form i))
		    ...
		    (let ((v (or vec (vector-copy form))))
		      ...
		      (loop (+ i 1) v)))))))
      (let loop ((lst olst))
	(cond ((hashtable-ref seen lst #f))
	      ((contain-identifier? lst)
	       (cond ((pair? lst)
		      (hashtable-set! seen lst lst)
		      (let ((a (loop (car lst))) (d (loop (cdr lst))))
			(if (and (eq? (car lst) a) (eq? (cdr lst) d))
			    lst
			    (source-info*! (cons a d) (source-info lst)))))
