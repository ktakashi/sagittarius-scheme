...
(define srfi-226-mapping
  '((prompts prompt)
    (continuations continuation)
    (shift-reset shift-reset)
    (inspection inspection)
    (continuation-marks continuation-mark)
    ...
    (times time)
    (threads thread)
    (thread-locals thread-local)
    (interrupts interrupt)))

(define (srfi-226-filename-generator file)
  ;; %3an+ -> n+
  (let ((normalized-name (default-filename-generator file)))
    (let*-values (((tdir base ext) (decompose-path normalized-name))
		  ;; drop /control
		  ((dir ignore0 ignore1) (decompose-path tdir)))
      (cond ((assq (string->symbol base) srfi-226-mapping) =>
	     (lambda (s)
	       (let ((filename (string-append (symbol->string (cadr s)) ".scm")))
		 (build-path dir filename))))
	    (else #f)))))
(define (default-filename-generator file)
  (regex-replace-all #/%3a/ file ""))

(define nested-srfis
  `(
    (#/%3a146[\/\\]hash/ ,default-filename-generator)
    (#/%3a160[\/\\]base/ ,default-filename-generator)
    (#/%3a160[\/\\][cfsu]\d{1,3}/ ,default-filename-generator)
    (#/%3a226[\/\\]control[\/\\].+\.scm/ ,srfi-226-filename-generator)
    ))

(define (create p pattern full-path? clean?
		:key (generate-filename default-filename-generator))
  ...
  (let ((files (find-files p :pattern pattern
    ...
    (dolist (file files)
      (cond ((generate-filename file) =>
		 ...
		 (let ((r7rs-srfi (file->srfi-libary new-file)))
		   (call-with-output-file new-file
		     (lambda (out)
		       (display +prelude+ out) (newline out)
		       (pp `(define-library ,r7rs-srfi
			      (export :all :export-reader-macro)
...
(define (main args)
  (with-args (cdr args) 
	...
	(create p (car rule) #t clean? :generate-filename (cadr rule))))
