...
label_CONST:
CASE(CONST) 
{{SgObject val=FETCH_OPERAND(PC(vm));{AC(vm)=(val);NEXT;}}}
...
label_LREF:
CASE(LREF) 
{INSN_VAL1(val1,c);{AC(vm)=(REFER_LOCAL(vm,val1));NEXT;}}

label_LSET:
CASE(LSET) 
{INSN_VAL1(val1,c);(SG_BOX(REFER_LOCAL(vm,val1)))->value=(AC(vm)),AC(vm)=(SG_UNDEF);NEXT;}

label_FREF:
CASE(FREF) 
{INSN_VAL1(val1,c);{AC(vm)=(INDEX_CLOSURE(vm,val1));NEXT;}}

label_FSET:
CASE(FSET) 
{INSN_VAL1(val1,c);(SG_BOX(INDEX_CLOSURE(vm,val1)))->value=(AC(vm)),AC(vm)=(SG_UNDEF);NEXT;}

label_GREF:
CASE(GREF) 
{{SgObject v=SG_FALSE;REFER_GLOBAL(vm,v);{AC(vm)=(v);NEXT;}}}

label_GSET:
CASE(GSET) 
{{SgObject var=FETCH_OPERAND(PC(vm));if (SG_GLOCP(var)){SG_GLOC_SET(SG_GLOC(var),AC(vm));}else if(SG_IDENTIFIERP(var)){{SgObject oldval=Sg_FindBinding(SG_IDENTIFIER_LIBRARY(var),SG_IDENTIFIER_NAME(var),SG_UNBOUND);if (SG_UNBOUNDP(oldval)){{Sg_AssertionViolation(SG_MAKE_STRING("set!"),Sg_Sprintf(UC("unbound variable %S"),SG_IDENTIFIER_NAME(var)),SG_IDENTIFIER_NAME(var));}}{SgObject g=Sg_MakeBinding(SG_IDENTIFIER_LIBRARY(var),SG_IDENTIFIER_NAME(var),AC(vm),0);(*((PC(vm))-(1)))=(SG_WORD(g));}}} else {Sg_Panic("[internal] GSET: gloc or identifier required");}}AC(vm)=(SG_UNDEF);NEXT;}
...
label_BOX:
CASE(BOX) 
{INSN_VAL1(val1,c);INDEX_SET(SP(vm),val1,make_box(INDEX(SP(vm),val1)));NEXT;}
...
label_ADD:
CASE(ADD) 
{{SgObject obj=INDEX(SP(vm),0);if ((SG_INTP(AC(vm)))&&(SG_INTP(obj))){{long n=(SG_INT_VALUE(obj))+(SG_INT_VALUE(AC(vm)));(SP(vm))--;if (((SG_INT_MIN)<=(n))&&((SG_INT_MAX)>=(n))){{AC(vm)=(SG_MAKE_INT(n));NEXT;}} else {{AC(vm)=(Sg_MakeBignumFromSI(n));NEXT;}}}} else {{SgObject v=obj;(SP(vm))--;{AC(vm)=(Sg_Add(v,AC(vm)));NEXT;}}}}}

label_ADDI:
CASE(ADDI) 
{INSN_VAL1(val1,c);if (SG_INTP(AC(vm))){{long n=(val1)+(SG_INT_VALUE(AC(vm)));if (((SG_INT_MIN)<=(n))&&((SG_INT_MAX)>=(n))){{AC(vm)=(SG_MAKE_INT(n));NEXT;}} else {{AC(vm)=(Sg_MakeBignumFromSI(n));NEXT;}}}} else {{AC(vm)=(Sg_Add(SG_MAKE_INT(val1),AC(vm)));NEXT;}}}

label_SUB:
CASE(SUB) 
{{SgObject obj=INDEX(SP(vm),0);if ((SG_INTP(AC(vm)))&&(SG_INTP(obj))){{long n=(SG_INT_VALUE(obj))-(SG_INT_VALUE(AC(vm)));(SP(vm))--;if (((SG_INT_MIN)<=(n))&&((SG_INT_MAX)>=(n))){{AC(vm)=(SG_MAKE_INT(n));NEXT;}} else {{AC(vm)=(Sg_MakeBignumFromSI(n));NEXT;}}}} else {{SgObject v=obj;(SP(vm))--;{AC(vm)=(Sg_Sub(v,AC(vm)));NEXT;}}}}}

label_SUBI:
CASE(SUBI) 
{INSN_VAL1(val1,c);if (SG_INTP(AC(vm))){{long n=(val1)-(SG_INT_VALUE(AC(vm)));if (((SG_INT_MIN)<=(n))&&((SG_INT_MAX)>=(n))){{AC(vm)=(SG_MAKE_INT(n));NEXT;}} else {{AC(vm)=(Sg_MakeBignumFromSI(n));NEXT;}}}} else {{AC(vm)=(Sg_Sub(SG_MAKE_INT(val1),AC(vm)));NEXT;}}}
...
label_MULI:
CASE(MULI) 
{INSN_VAL1(val1,c);{AC(vm)=(Sg_Mul(SG_MAKE_INT(val1),AC(vm)));NEXT;}}

label_DIV:
CASE(DIV) 
{{SgObject obj=INDEX(SP(vm),0);int exact=(Sg_ExactP(obj))&&(Sg_ExactP(AC(vm)));if (((exact)&&(SG_VM_IS_SET_FLAG(vm,SG_R6RS_MODE)))&&(Sg_ZeroP(AC(vm)))){{Sg_AssertionViolation(SG_INTERN("/"),SG_MAKE_STRING("undefined for 0"),SG_LIST2(obj,AC(vm)));}} else {{SgObject v=obj;(SP(vm))--;{AC(vm)=(Sg_Div(v,AC(vm)));NEXT;}}}}}

label_DIVI:
CASE(DIVI) 
{INSN_VAL1(val1,c);{AC(vm)=(Sg_Div(SG_MAKE_INT(val1),AC(vm)));NEXT;}}
...
label_TEST:
CASE(TEST) 
{if (SG_FALSEP(AC(vm))){{SgObject n=PEEK_OPERAND(PC(vm));(PC(vm))+=(SG_INT_VALUE(n));}} else {(PC(vm))++;}NEXT;}
...
label_SHIFTJ:
CASE(SHIFTJ) 
{INSN_VAL2(val1,val2,c);SP(vm)=(shift_args((FP(vm))+(val2),val1,SP(vm)));NEXT;}
...
label_APPLY:
CASE(APPLY) 
{INSN_VAL2(val1,val2,c);{int rargc=Sg_Length(AC(vm));int nargc=(val1)-(2);SgObject proc=INDEX(SP(vm),nargc);SgObject* fp=(SP(vm))-((val1)-(1));if ((rargc)<(0)){{{Sg_AssertionViolation(SG_INTERN("apply"),SG_MAKE_STRING("improper list not allowed"),AC(vm));}}}shift_args(fp,nargc,SP(vm));if ((rargc)==(0)){(SP(vm))--;if (val2){{SP(vm)=(shift_args(FP(vm),nargc,SP(vm)));}}AC(vm)=(proc);c=(MERGE_INSN_VALUE1(CALL,nargc));goto label_CALL;} else {INDEX_SET(SP(vm),0,AC(vm));if (val2){{SP(vm)=(shift_args(FP(vm),(nargc)+(1),SP(vm)));}}c=(MERGE_INSN_VALUE1(CALL,(nargc)+(1)));AC(vm)=(proc);goto tail_apply_entry;}}}
...
label_LOCAL_CALL:
CASE(LOCAL_CALL) 
{CHECK_STACK(SG_CLOSURE_MAX_STACK(AC(vm)),vm);{INSN_VAL1(val1,c);if ((SG_VM_LOG_LEVEL(vm,SG_TRACE_LEVEL))&&(((vm)->state)==(RUNNING))){{Sg_Printf((vm)->logPort,UC(";; calling %S\n"),AC(vm));}}{SgCodeBuilder* cb=(SG_CLOSURE(AC(vm)))->code;CL(vm)=(AC(vm)),PC(vm)=((cb)->code),FP(vm)=((SP(vm))-(val1));}}NEXT;}

label_TAIL_CALL:
CASE(TAIL_CALL) 
{{INSN_VAL1(val1,c);SP(vm)=(shift_args(FP(vm),val1,SP(vm)));}goto label_CALL;}

label_LOCAL_TAIL_CALL:
CASE(LOCAL_TAIL_CALL) 
{CHECK_STACK(SG_CLOSURE_MAX_STACK(AC(vm)),vm);{INSN_VAL1(val1,c);SP(vm)=(shift_args(FP(vm),val1,SP(vm)));}{INSN_VAL1(val1,c);if ((SG_VM_LOG_LEVEL(vm,SG_TRACE_LEVEL))&&(((vm)->state)==(RUNNING))){{Sg_Printf((vm)->logPort,UC(";; calling %S\n"),AC(vm));}}{SgCodeBuilder* cb=(SG_CLOSURE(AC(vm)))->code;CL(vm)=(AC(vm)),PC(vm)=((cb)->code),FP(vm)=((SP(vm))-(val1));}}NEXT;}
...
label_FRAME:
CASE(FRAME) 
{{SgObject n=FETCH_OPERAND(PC(vm));PUSH_CONT(vm,(PC(vm))+((SG_INT_VALUE(n))-(1)));}NEXT;}

label_ENTER:
CASE(ENTER) 
{INSN_VAL1(val1,c);FP(vm)=((SP(vm))-(val1));NEXT;}
...
label_LIBRARY:
CASE(LIBRARY) 
{{SgObject lib=Sg_FindLibrary(FETCH_OPERAND(PC(vm)),FALSE);(vm)->currentLibrary=(((SgLibrary* )(lib)));}NEXT;}

label_CAR:
CASE(CAR) 
{if ((!(SG_PAIRP(AC(vm))))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("car"),SG_MAKE_STRING("pair"),AC(vm),SG_NIL);}}}{AC(vm)=(SG_CAR(AC(vm)));NEXT;}}

label_CDR:
CASE(CDR) 
{if ((!(SG_PAIRP(AC(vm))))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("cdr"),SG_MAKE_STRING("pair"),AC(vm),SG_NIL);}}}{AC(vm)=(SG_CDR(AC(vm)));NEXT;}}
...
label_VEC_REF:
CASE(VEC_REF) 
{{SgObject obj=INDEX(SP(vm),0);if ((!(SG_VECTORP(obj)))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("vector-ref"),SG_MAKE_STRING("vector"),obj,SG_NIL);}}}if ((!(SG_INTP(AC(vm))))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("vector-ref"),SG_MAKE_STRING("fixnum"),AC(vm),SG_NIL);}}}{int index=SG_INT_VALUE(AC(vm));if (((index)>=(SG_VECTOR_SIZE(obj)))||((index)<(0))){{{Sg_AssertionViolation(SG_INTERN("vector-ref"),SG_MAKE_STRING("index out of range"),SG_LIST2(obj,AC(vm)));}}}(SP(vm))--;{AC(vm)=(SG_VECTOR_ELEMENT(obj,index));NEXT;}}}}
...
label_LREF_PUSH:
CASE(LREF_PUSH) 
{INSN_VAL1(val1,c);PUSH(SP(vm),REFER_LOCAL(vm,val1));NEXT;}

label_FREF_PUSH:
CASE(FREF_PUSH) 
{INSN_VAL1(val1,c);PUSH(SP(vm),INDEX_CLOSURE(vm,val1));NEXT;}

label_GREF_PUSH:
CASE(GREF_PUSH) 
{{SgObject v=SG_FALSE;REFER_GLOBAL(vm,v);{PUSH(SP(vm),v);NEXT;}}}
...
label_CONSTI_PUSH:
CASE(CONSTI_PUSH) 
{INSN_VAL1(val1,c);PUSH(SP(vm),SG_MAKE_INT(val1));NEXT;}

label_GREF_CALL:
CASE(GREF_CALL) 
{{SgObject v=SG_FALSE;REFER_GLOBAL(vm,v);{AC(vm)=(v);}}}
{goto label_CALL;}
...
label_SET_CAR:
CASE(SET_CAR) 
{{SgObject obj=INDEX(SP(vm),0);if ((!(SG_PAIRP(obj)))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("set-car!"),SG_MAKE_STRING("pair"),obj,SG_NIL);}}}if (Sg_ConstantLiteralP(obj)){{{Sg_AssertionViolation(SG_INTERN("set-car!"),SG_MAKE_STRING("attempt to modify constant literal"),obj);}}}SG_SET_CAR(obj,AC(vm));(SP(vm))--;{AC(vm)=(SG_UNDEF);NEXT;}}}

label_SET_CDR:
CASE(SET_CDR) 
{{SgObject obj=INDEX(SP(vm),0);if ((!(SG_PAIRP(obj)))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("set-cdr!"),SG_MAKE_STRING("pair"),obj,SG_NIL);}}}if (Sg_ConstantLiteralP(obj)){{{Sg_AssertionViolation(SG_INTERN("set-cdr!"),SG_MAKE_STRING("attempt to modify constant literal"),obj);}}}SG_SET_CDR(obj,AC(vm));(SP(vm))--;{AC(vm)=(SG_UNDEF);NEXT;}}}
...
label_CAR_PUSH:
CASE(CAR_PUSH) 
{if ((!(SG_PAIRP(AC(vm))))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("car"),SG_MAKE_STRING("pair"),AC(vm),SG_NIL);}}}{PUSH(SP(vm),SG_CAR(AC(vm)));NEXT;}}

label_CDR_PUSH:
CASE(CDR_PUSH) 
{if ((!(SG_PAIRP(AC(vm))))){{{Sg_WrongTypeOfArgumentViolation(SG_INTERN("cdr"),SG_MAKE_STRING("pair"),AC(vm),SG_NIL);}}}{PUSH(SP(vm),SG_CDR(AC(vm)));NEXT;}}
...
label_CONST_RET:
CASE(CONST_RET) 
{{SgObject val=FETCH_OPERAND(PC(vm));{AC(vm)=(val);RET_INSN();NEXT;}}}
#endif /* VM_LOOP */
