...
static SgObject nullstring_ref(SgObject *args, int argc, void *data_)
{
  ...
  argumentAsFixnum(1, k_scm, k);
  {
    SgObject SG_RETURN = SG_UNDEF;
    if (k < 0) {
      Sg_WrongTypeOfArgumentViolation(SG_INTERN("string-ref"), Sg_MakeString(UC("non-negative exact integer required"), SG_LITERAL_STRING), SG_MAKE_INT(k), SG_LIST2(s, SG_MAKE_INT(k)));
      return SG_UNDEF;
;
    }
;
...
static SgObject nullsubstring(SgObject *args, int argc, void *data_)
{
  ...
  argumentAsFixnum(2, end_scm, end);
  {
    SgObject SG_RETURN = SG_UNDEF;
    if (start < 0) {
      Sg_WrongTypeOfArgumentViolation(SG_INTERN("substring"), Sg_MakeString(UC("non-negative exact integer required"), SG_LITERAL_STRING), SG_MAKE_INT(start), SG_LIST3(s, SG_MAKE_INT(start), SG_MAKE_INT(end)));
      return SG_UNDEF;
;
    }
;
    if (end < 0) {
      Sg_WrongTypeOfArgumentViolation(SG_INTERN("substring"), Sg_MakeString(UC("non-negative exact integer required"), SG_LITERAL_STRING), SG_MAKE_INT(end), SG_LIST3(s, SG_MAKE_INT(start), SG_MAKE_INT(end)));
      return SG_UNDEF;
;
    }
;
...
static SgObject nullstring_set21(SgObject *args, int argc, void *data_)
{
  ...
  argumentRef(2, c);
  {
    SgObject SG_RETURN = SG_UNDEF;
    if (k < 0) {
      Sg_WrongTypeOfArgumentViolation(SG_INTERN("string-set!"), Sg_MakeString(UC("non-negative exact integer required"), SG_LITERAL_STRING), SG_MAKE_INT(k), SG_LIST3(s, SG_MAKE_INT(k), c));
      return SG_UNDEF;
;
    }
;
