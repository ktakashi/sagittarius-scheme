...
;
          {
            SgObject cgen_30;
            SG_FOR_EACH(cgen_30,SG_CDAR(fp)) {
              {
                SgObject vp = SG_CAR(cgen_30);
                if (SG_EQ(name, SG_CAR(vp))) {
                  return SG_CDR(vp);
                }
;
...
;
static SgObject _sagittarius_vm_p1env_frame_lookup(SgObject *args, int argc, void *data_)
{
  DeclareProcedureName("p1env-frame-lookup");
  SgObject p1env_scm;
  SgVector *p1env;
  SgObject name_scm;
  SgSymbol *name;
  ...
  argumentAsVector(0, p1env_scm, p1env);
  argumentAsSymbol(1, name_scm, name);
  argumentAsFixnum(2, lookup_as_scm, lookup_as);
  {
    SgObject SG_RETURN = SG_UNDEF;
    SG_RETURN = (Sg_P1envFrameLookup(name, p1env, lookup_as));
    return SG_RETURN;
  }
}
static SG_DEFINE_SUBR(_sagittarius_vm_p1env_frame_lookup_Stub, 3, 0, _sagittarius_vm_p1env_frame_lookup, SG_FALSE, NULL);
...
void Sg__Init_sagittarius_vm()
{
  ...
  SG_PROCEDURE_NAME(&_sagittarius_vm_p1env_frame_lookup_Stub) = Sg_MakeString(UC("p1env-frame-lookup"), SG_LITERAL_STRING);
  Sg_InsertBinding(lib, Sg_Intern(Sg_MakeString(UC("p1env-frame-lookup"), SG_LITERAL_STRING)), SG_OBJ(&_sagittarius_vm_p1env_frame_lookup_Stub));
