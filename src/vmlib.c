...
;
          {
            SgObject cgen_43;
            SG_FOR_EACH(cgen_43,SG_CDAR(fp)) {
              {
                SgObject vp = SG_CAR(cgen_43);
...
static SgObject _sagittarius_vm_p1env_toplevel3f(SgObject *args, int argc, void *data_)
{
  ...
  argumentRef(0, p1env);
  {
    ...
    {
      SgObject cgen_44;
      SG_FOR_EACH(cgen_44,SG_VECTOR_ELEMENT(p1env, 1)) {
        {
          SgObject fp = SG_CAR(cgen_44);
...
;
          {
            SgObject cgen_45;
            SG_FOR_EACH(cgen_45,SG_CDAR(fp)) {
              {
                SgObject vp = SG_CAR(cgen_45);
                if (SG_EQ(name, SG_CAR(vp))) {
                  return SG_CDR(vp);
                }
;
