...
;
static SgObject _sagittarius_syntax_case_literals(SgObject *args, int argc, void *data_)
{
  DeclareProcedureName("syntax-case-literals");
  SgObject syncase_scm;
  SgSyntaxCase *syncase;
  checkArgumentLength(1);
  argumentAsSyntaxCase(0, syncase_scm, syncase);
  {
    SgObject SG_RETURN = SG_UNDEF;
    SG_RETURN = (SG_SYNTAX_CASE(syncase)->literals);
    return SG_RETURN;
  }
}
static SG_DEFINE_SUBR(_sagittarius_syntax_case_literals_Stub, 1, 0, _sagittarius_syntax_case_literals, SG_FALSE, NULL);

;
static SgObject _sagittarius_syntax_case_patterns(SgObject *args, int argc, void *data_)
{
  DeclareProcedureName("syntax-case-patterns");
  SgObject syncase_scm;
  SgSyntaxCase *syncase;
  checkArgumentLength(1);
  argumentAsSyntaxCase(0, syncase_scm, syncase);
  {
    SgObject SG_RETURN = SG_UNDEF;
    SG_RETURN = (SG_SYNTAX_CASE(syncase)->patterns);
    return SG_RETURN;
  }
}
static SG_DEFINE_SUBR(_sagittarius_syntax_case_patterns_Stub, 1, 0, _sagittarius_syntax_case_patterns, SG_FALSE, NULL);
...
void Sg__Init_sagittarius()
{
  ...
  SG_PROCEDURE_NAME(&_sagittarius_syntax_case_patterns_Stub) = Sg_MakeString(UC("syntax-case-patterns"), SG_LITERAL_STRING);
  Sg_InsertBinding(lib, Sg_Intern(Sg_MakeString(UC("syntax-case-patterns"), SG_LITERAL_STRING)), SG_OBJ(&_sagittarius_syntax_case_patterns_Stub));
  ...
  SG_PROCEDURE_NAME(&_sagittarius_syntax_case_literals_Stub) = Sg_MakeString(UC("syntax-case-literals"), SG_LITERAL_STRING);
  Sg_InsertBinding(lib, Sg_Intern(Sg_MakeString(UC("syntax-case-literals"), SG_LITERAL_STRING)), SG_OBJ(&_sagittarius_syntax_case_literals_Stub));
