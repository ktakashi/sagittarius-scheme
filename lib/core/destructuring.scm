;; -*- scheme -*-
;; Sagittarius
(library (core destructuring helper)
    (export generate-temporary-symbol)
    (import (core)
	    (sagittarius))
  (define generate-temporary-symbol gensym)
)
;; from Ypsilon
  ...
  (define reorder-tests
    (lambda (lst preds)
      (let loop ((lst lst) (preds preds) (acc '()))
        (if (null? preds)
            (append acc lst)
            (receive (hits rest) (partition (lambda (e) (memq (car e) (car preds))) lst)
        ...
        ((?_ ?expr ?clauses ...)
         (let ((datum (generate-temporary-symbol))
           ...
           (let ((code
                   ...
                   (reorder
                    (map (lambda (clause)
                           (syntax-case clause ()
                             ((?pat)
                              (let ((pat (syntax->datum #'?pat)))
                                (receive (match inits vars) (compile-match ren mem pat datum '() '() '())
             ...
             (receive (shares others) (partition car code)
               (let ((subexprs (map (lambda (e) `(,(car e) (and ,@(cadr e)))) shares))
                 ...
                 (with-syntax
                     ((?datum (datum->syntax #'k datum))
                      ...
                      ((?dispatch ...)
                       (map (lambda (clause)
                              (receive (tests vars inits body fender) (apply values clause)
                                (with-syntax
                                    (((?vars ...) (map (lambda (e) (datum->syntax #'?_ e)) vars))
                                     ((?inits ...) (map (lambda (e) (datum->syntax #'k e)) inits))
