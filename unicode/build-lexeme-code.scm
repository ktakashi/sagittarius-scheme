...
(library (anonymous)
  ...
  (define ucd-file
    (lambda (filename)
      (string-append (current-directory) "/unicode-5.2.0/" filename)))
		    ...
		    (put-string output "// -*- C -*- \n")
                    ...
                    (let ((bv bv-c))
                      (let ((bytes (bytevector-length bv)))
                        (put-string output (format "static const uint8_t constituent[~a] = {" bytes))
                        (let loop ((c 0))
                    ...
                    (let ((bv bv-s))
                      (let ((bytes (bytevector-length bv)))
                        (put-string output (format "static const uint8_t subsequent[~a] = {" bytes))
                        (let loop ((c 0))
                          (if (zero? (mod c 16)) (put-byte output (char->integer #\linefeed)))
                          (let ((b (bytevector-u8-ref bv c)))
                            (cond ((= (+ c 1) bytes)
