...
(test-begin "regular expression test")
...
(let ((match&list (lambda (pat text . flags)
		    (define (->string/false bv)
		      (and bv (utf8->string bv)))
		    (and-let* ((r (generic-match&list 
				   regex-looking-at pat 
				   (string->utf8 text) flags)))
		      (map ->string/false r)))))
  ;; basics
  (test-equal "a" '("a")
	      (match&list "a" "a"))
...
(let ((replace (lambda (proc regex text replacement)
		 (utf8->string
		  (proc regex (string->utf8 text) (string->utf8 replacement))))))

  ;; binary replace
  (test-equal "regex-replace-first" "abc|def|ghi"
	      (replace regex-replace-first (regex "def|DEF") 
		       "abcdefghi" 
		       "|$0|"))
  ;; pre/post ($p/$P)
  (test-equal "regex-replace-first" "abc|abcdefghi|ghi"
  ...
  (test-equal "regex-replace-first"
	      "abraabra**brabra**brabrabracadabrabrabra"
	      (replace regex-replace-first
		       (regex "a((bra)+)cadabra")
		       "abraabraabrabracadabrabrabrabracadabrabrabra"
		       "**$1**"))

  (test-equal "regex-replace-all" "abraabra**brabra**br**brabra**brabra"
	      (replace regex-replace-all
		       (regex "a((bra)+)cadabra")
		       "abraabraabrabracadabrabrabrabracadabrabrabra"
		       "**$1**")))
