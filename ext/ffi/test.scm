 ...
 (sagittarius.ffi
  ...
  (when (support-variable-length?)
	...
	(let ((r (va-fn result 4 1 #t "abcdef" 4)))
	  ...
	  (test-equal "result(3)" 3 (pointer->string (deref result 2)))
	  (test-equal "result(4)" 4 (pointer->integer (deref result 3)))
	  )
	(let ((r (va-fn result 4 1 2 3 4 5 6 7)))
	  (test-equal "varargs" 4 r)
	  (test-equal "result(1~)" 1 (deref result 0))
	  (test-equal "result(2~)" 2 (deref result 1))
	  (test-equal "result(3~)" 3 (deref result 2))
	  (test-equal "result(4~)" 4 (deref result 3)))
	)))
  )
 (else
  #t))
