...
@desc{This library has Sagittarius specific functionalities such as extra file
...
@code{#!no-overwite} or other directives, then compiler raises an error.
}

@define[Syntax]{@name{receive} @args{formals expression body}}
@desc{[SRFI-8] @var{formals} and @var{body} are the same as @code{lambda}.
...
Users can check possible @var{feature-identifier}s via @code{cond-features}
procedure.
}
