...
@desc{The @code{(rnrs io ports (6))}library defines an I/O layer for
conventional, imperative buffered input and output. A @var{port} represents
...
The @code{(rnrs io ports (6))} library also distinguishes between
@var{binary ports}, which are sources or sinks for uninterpreted bytes,
and @var{textual ports}, which are sources or sinks for characters and strings.
...
valid file options. A @var{file-options} parameter name means that the
...
object returned by @code{(file-options)} specifies that the file is created if
it does not exist and an exception with condition type
@code{&i/o-file-already-exists} is raised if it does exist. The following
...
A @var{binary port} is a port that supports binary I/O, does not have an
associated transcoder and does not support textual I/O. A @var{textual} 
...
@desc{[R6RS] An exception with this type is raised when one of the operations
...
one of @code{ignore}, @code{raise}, and @code{replace}. The form evaluates to
...
encoding, and the error-handling mode is @code{ignore}, an appropriate number
of bytes of the invalid encoding are ignored and decoding continues with the
following bytes. If the error-handling mode is @code{replace}, the replacement
...
for transcoders whose codec is one of the Unicode encodings, but is the @code{?}
...
@define[Function]{@name{make-transcoder}
 @args{codec :optional eol-style handling-mode}}
@desc{[R6RS] @var{Codec} must be a codec; @var{eol-style}, if present, an
eol-style symbol; and @var{handling-mode}, if present, an error-handling-mode
symbol. @var{Eol-style} may be omitted, in which case it defaults to the native
end-of-line style of the underlying platform. @var{Handling-mode} may be
omitted, in which case it defaults to @code{replace}. The result is a
...
@desc{[R6RS] These are accessors for transcoder objects; when applied to a
transcoder returned by @code{make-transcoder}, they return the @var{codec},
...
@desc{[R6RS] Returns the transcoder associated with @var{port} if port is
textual and has an associated transcoder, and returns #f if @var{port} is
...
procedure calls @var{proc} with @var{port} as an argument. If @var{proc}
returns, @var{port} is closed automatically and the values returned by
@var{proc} are returned. If @var{proc} does not return, @var{port} is not
closed automatically, except perhaps when it is possible to prove that
@var{port} will never again be used for an input or output operation. 
}
...
@desc{Returns #t if the argument is an input port (or a combined input and
output port), and returns #f otherwise.}

@define[Function]{@name{port-eof?} @args{input-port}}
@desc{[R6RS] Returns #t if the @code{lookahead-u8} procedure (if
@var{input-port} is a binary port) or the @code{lookahead-char} procedure (if
@var{input-port} is a textual port) would return the end-of-file object, and
#f otherwise.
}
...
of the port. So does @code{open-string-input-port}.
}
...
@var{port} as a default port for input. Otherwise it returns a default input
port.}

@define[Function]{@name{make-custom-binary-input-port}
 @args{id read! get-position set-position! close :optional (ready #f)}}
@desc{[R6RS+] Returns a newly created binary input port whose byte source is an
arbitrary algorithm represented by the read! procedure. @var{Id} must be a
string naming the new port, provided for informational purposes only.
@var{Read!} must be a procedure and should behave as specified below; it will
...
obtain up to @var{count} bytes from the byte source, and should write those
bytes into @var{bytevector} starting at index @var{start}. The @var{read!}
...
@p{@var{Pos} will be a non-negative exact integer object. The
@var{set-position!} procedure (if supplied) should set the position of the
input port to @var{pos}. If not supplied, the custom port will not support the
@var{set-port-position!} operation.}}
@item{(@var{close})
@p{The @var{close} procedure (if supplied) should perform any actions that are
necessary when the input port is closed.}}
@item{(@var{ready})
@p{The @var{ready} procedure (if supplied) should indicate the port data are
ready or not.}}
]
}
...
necessary when the input port is closed.}}
@item{(@var{ready})
@p{The @var{ready} procedure (if supplied) should indicate the port characters
are ready or not.}}
]
}
...
@define[Function]{@name{make-custom-binary-input/output-port}
 @args{id read! write! get-position set-position! close :optional (ready #f)}}
...
@var{write!} procedures. @var{Id} must be a string naming the new port,
provided for informational purposes only. @var{Read!} and @var{write!} must
...
@define[Function]{@name{make-custom-textual-input/output-port}
 @args{id read! write! get-position set-position! close :optional (ready #f)}}
@desc{[R6RS+] Returns a newly created textual input/output port whose byte
source and sink are arbitrary algorithms represented by the @var{read!}
and @var{write!} procedures. @var{Id} must be a string naming the new port,
provided for informational purposes only. @var{Read!} and @var{write!} must
be procedures, and should behave as specified for the
@code{make-custom-textual-input-port} and
