...
If @var{var-spec} is a list, the default value is the result of evaluation of the
second element of the list.

In the latter case the second element is only evaluated when there are not enough
arguments. The binding proceeds in the order of @var{var-spec}, so the second
element may refer to the bindings of previous @var{var-spec}.

In the second form, @var{restvar} must be a symbol and bound to the list of values
whatever left from @var{restargs} after binding to @var{var-spec}.

It is not an error if @var{restarg} has more values than @var{var-specs}. The extra
values are simply ignored in the first form. 
}
...
when @var{restarg} contains a keyword argument that is not listed in @var{var-specs}.
When you want to allow keyword arguments other than listed in @var{var-specs}, use
the second form.

In the second form, @var{restvar} must be either a symbol or #f. If it is a symbol,
it is bound to a list of keyword arguments that are not processed by @var{var-specs}.
If it is #f, such keyword arguments are just ignored.
...
@var{var-specs}. So each @var{expr} can refer to the variables bound by preceding
@var{var-specs}.

These let-keywords and let-keywords* are originally from Gauche. 
}

@define[Macro]{@name{define-with-key} @args{name value}}
@define[Macro]{@name{define-with-key}
 @args{(name arg @dots{} :key (ver-spec @dots{})) body @dots{}}}
@define[Macro]{@name{define-with-key}
 @args{(name arg @dots{} :key (ver-spec @dots{} . restvar)) body @dots{}}}
@define[Macro]{@name{define-with-key}
 @args{(name arg @dots{} :key (ver-spec @dots{}) :allow-other-keys . opt @dots{}) body @dots{}}}
@define[Macro]{@name{define-with-key}
 @args{(name arg @dots{} :key (ver-spec @dots{} . restvar) :allow-other-keys . opt @dots{}) body @dots{}}}
@desc{The @code{define-with-key} is similar with the form @code{define}, it uses
@code{let-keywors*} inside. If the third or fourth form is used, it can receive
non-defined keywords. The macro will be expanded approximately like this:

@codeblock{(define-with-key (name arg0 arg1 :key key1 (key2 'value) :allow-other-keys opt) body @dots{})}
=>
@codeblock{
...
If the first form is used, it is the same as @code{define}.
}

@define[Macro]{@name{begin0} @args{exp0 exp1 @dots{}}}
@desc{Evaluate @var{exp0}, @var{exp1}, @dots{}, then returns the result(s) of @var{exp0}.
}
